...
class MemberDocController extends Controller
{
  ...
  public function Member_Doc_datatable(Request $rs)
  {
    ...
    $get_member_doc = DB::table('customers')
      ->whereRaw('(regis_doc1_status != 1 or regis_doc2_status != 1 )')
      ->orderBy('order_regis_file_date');
    ...
    return $sQuery
      ...
      ->rawColumns(['regis_doc1_status', 'regis_doc2_status'])
      ->make(true);
  }
  ...
  public function Member_Doc_update(Request $rs)
  {
    ...
    if ($rs->regis_doc_status == "2") {
      ...
      if ($get_db_register_file->type == '1') {
        ...
        DB::table('customers')
          ->where('id', $get_db_register_file->customer_id)
          ->update(['regis_doc2_status' => 1]);
      }else{
        return redirect('admin/MemberDoc')->withError('อนุมัติเอกสารไม่สำเร็จ');
      }

      \App\Http\Controllers\Admin\MemberDocController::check_customer_doc_approve($get_db_register_file->customer_id);

      return redirect('admin/MemberDoc')->withSuccess('อนุมัติเอกสารสำเร็จ');
      ...
      $get_db_register_file = DB::table('register_files')
        ...
        if ($get_db_register_file->type == '1') {
            ...
            DB::table('customers')
              ->where('id', $get_db_register_file->customer_id)
              ->update(['regis_doc1_status' => 2]);
          } elseif ($get_db_register_file->type == '2') {

            DB::table('customers')
              ->where('id', $get_db_register_file->customer_id)
              ->update(['regis_doc2_status' => 2]);
          }else{
            return redirect('admin/MemberDoc')->withError('ยกเลิกเอกสารไม่สำเร็จ');
          }

      return redirect('admin/MemberDoc')->withSuccess('ยกเลิกรายการสำเร็จ');
    }
  }
  ...
  public function Member_Doc_view(Request $rs)
  {
    ...
    $get_member_doc = DB::table('register_files')
      ->select('register_files.*', 'customers.name', 'customers.user_name', 'customers.last_name', 'customers.id_card')
